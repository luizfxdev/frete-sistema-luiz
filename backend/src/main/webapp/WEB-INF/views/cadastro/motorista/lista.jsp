...
<style>
  ...
  .nl-kpi-icon.red { background: #fee2e2; color: #dc2626; }
  ...
  .badge-vencida { background: #fee2e2; color: #991b1b; margin-left: 0.5rem; }
...
<main class="nl-main">
  <div class="nl-container">
    <div class="nl-page-header">
      <h1><i class="bi bi-person-badge-fill" style="color: #3b82f6; margin-right: 0.75rem;"></i>Motoristas</h1>
      <a href="${pageContext.request.contextPath}/motoristas/novo" class="btn btn-primary">
    ...
    <div class="nl-kpi-row">
      <div class="nl-kpi">
        <div class="nl-kpi-icon blue"><i class="bi bi-person-badge-fill"></i></div>
        <div>
          <div class="nl-kpi-label">Total</div>
          <div class="nl-kpi-value">${totalMotoristas != null ? totalMotoristas : 0}</div>
          <div class="nl-kpi-sub">motoristas</div>
        </div>
      </div>
      <div class="nl-kpi">
        ...
        <div>
          ...
          <div class="nl-kpi-sub">disponíveis</div>
        </div>
      </div>
      <div class="nl-kpi">
        <div class="nl-kpi-icon amber"><i class="bi bi-pause-circle-fill"></i></div>
        <div>
          <div class="nl-kpi-label">Suspensos</div>
          <div class="nl-kpi-value">${totalSuspensos != null ? totalSuspensos : 0}</div>
          <div class="nl-kpi-sub">fora de operação</div>
        </div>
      </div>
      <div class="nl-kpi">
        <div class="nl-kpi-icon red"><i class="bi bi-exclamation-triangle-fill"></i></div>
        <div>
          <div class="nl-kpi-label">CNH Vencida</div>
          <div class="nl-kpi-value">${totalCnhVencida != null ? totalCnhVencida : 0}</div>
          <div class="nl-kpi-sub">requer atenção</div>
        </div>
      </div>
    </div>

    <form method="get" action="${pageContext.request.contextPath}/motoristas" class="nl-filter-card">
      <div class="nl-filter-row">
        <i class="bi bi-search"></i>
        <input type="text" name="filtro" value="<c:out value='${filtro}'/>" placeholder="Filtrar por nome ou CPF…">
        <button type="submit" class="btn btn-secondary">
          <i class="bi bi-funnel-fill"></i> Filtrar
        </button>
        <c:if test="${not empty filtro}">
          <a href="${pageContext.request.contextPath}/motoristas" class="btn btn-sm btn-secondary">
    ...
    <div class="nl-table-card">
      <table class="nl-table">
        ...
        <tbody>
          <c:forEach var="m" items="${motoristas}">
            <tr>
              <td class="nl-td-name"><c:out value="${m.nome}"/></td>
              <td class="nl-td-mono"><c:out value="${fn:substring(m.cpf, 0, 3)}.${fn:substring(m.cpf, 3, 6)}.${fn:substring(m.cpf, 6, 9)}-${fn:substring(m.cpf, 9, 11)}"/></td>
              <td style="font-size: 0.82rem;">
                <span style="font-weight: 600; color: #3b82f6;"><c:out value="${m.cnhCategoria}"/></span>
                <span style="color: #94a3b8; margin: 0 0.2rem;">·</span>
                <c:out value="${m.cnhNumero}"/>
              </td>
              <td>
                <c:out value="${m.cnhValidade}"/>
                <c:if test="${m.cnhVencida}">
                  <span class="nl-badge badge-vencida">Vencida</span>
                </c:if>
              </td>
              <td><c:out value="${m.tipoVinculo}"/></td>
              <td>
                <span class="nl-badge ${fn:toLowerCase(m.status.name()) == 'ativo' ? 'badge-ativo' : (fn:toLowerCase(m.status.name()) == 'inativo' ? 'badge-inativo' : 'badge-suspenso')}">
                  <c:out value="${m.status}"/>
                </span>
              </td>
              <td style="text-align: center;">
                <div class="nl-actions">
                  <a href="${pageContext.request.contextPath}/motoristas/editar/${m.id}" class="btn btn-sm btn-secondary">
                    <i class="bi bi-pencil-fill"></i> Editar
                  </a>
                  <form method="post" action="${pageContext.request.contextPath}/motoristas/excluir/${m.id}" style="display: inline; margin: 0;">
                    <button type="submit" class="btn btn-sm btn-danger" data-confirm="Confirmar exclusão do motorista?">
      ...
      <c:if test="${totalPaginas > 1}">
        <div class="nl-pagination">
          <c:if test="${pagina > 1}">
            <a href="${pageContext.request.contextPath}/motoristas?pagina=1<c:if test='${not empty filtro}'>&amp;filtro=<c:out value='${filtro}'/></c:if>">« Primeira</a>
            <a href="${pageContext.request.contextPath}/motoristas?pagina=${pagina - 1}<c:if test='${not empty filtro}'>&amp;filtro=<c:out value='${filtro}'/></c:if>">‹ Anterior</a>
          </c:if>
          ...
          <c:if test="${pagina < totalPaginas}">
            <a href="${pageContext.request.contextPath}/motoristas?pagina=${pagina + 1}<c:if test='${not empty filtro}'>&amp;filtro=<c:out value='${filtro}'/></c:if>">Próxima ›</a>
            <a href="${pageContext.request.contextPath}/motoristas?pagina=${totalPaginas}<c:if test='${not empty filtro}'>&amp;filtro=<c:out value='${filtro}'/></c:if>">Última »</a>
          </c:if>
        </div>
      </c:if>
    </div>
