...
@WebServlet("/api/dashboard/rotas")
public class DashboardRotasApi extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try {
            int limite = parseInt(req.getParameter("limite"), 5);
            JsonResponse.ok(resp, dashboardBO.rotasMaisMovimentadas(limite));
        } catch (NegocioException e) {
            JsonResponse.erro(resp, HttpServletResponse.SC_BAD_REQUEST, e.getMessage());
        } catch (Exception e) {
            JsonResponse.erro(resp, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Erro ao carregar rotas.");
        }
    }
